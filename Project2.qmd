...
abstract: |
  Objective: This study aims to compare the performance of Lasso and Stepwise logistic regression models in predicting critical outcomes (death or tracheostomy) in neonatal care, focusing on variable importance and model calibration.
...
The summary of variables definition can be found in @ref(table-variable). We separated the 30 variables into 5 groups, which is easy to make regression later.
...
**Outcome Variable** The dataset under consideration includes two critical variables: `Trach`, indicating whether the infants underwent tracheostomy, and `Death`, denoting mortality status. For the purposes of this project, these variables will be amalgamated to create a composite outcome measure `Outcome`. This derived binary variable will serve as the dependent variable in our predictive models, enabling us to ascertain the likelihood of either tracheostomy or death in the infant population under study. We created the composite variable because these two variables both denote a sever outcome for the infants.
...
#| warning: false
set.seed(123)
data <- read.csv("project2_data/project2.csv")
...
\begin{equation}
\begin{aligned}
\log\left(\frac{p}{1 - p}\right) = \, & \beta_0 +b_{center_i}+ \beta_1 \times \text{center} + \beta_2 \times \text{mat\_ethn} + \beta_3 \times \text{bw} + \\
...
\end{equation}
```
where, $N$ is the total number of predictions (or the size of your dataset). $f_i$is the predicted probability for instance $o_i$is the actual outcome for instance $i$, typically 0 or 1 in binary classification.
...
# Load the codebook
codebook <- readxl::read_excel("project2_data/project2_codebook.xlsx")
...
save(data.new,file = "project2_data/data.new.Rda")
```
...
load(file = "project2_data/data.new.Rda")
...
save(completed_data,file = "project2_data/completed_data.Rda")
```
...
lasso_model_list <- lapply(train_sets,FUN = function(train_set){
  Lasso_models(train_set = train_set, myformula = formula_fixed)
})
...
Model 1 demonstrated a mean CV Brier Score of 0.093 and an impressive lowest Brier Score of 0.039, suggesting a high potential for accuracy under optimal conditions despite less consistent average performance. Model 2, with a mean CV Brier Score of 0.08827815 and a lowest Brier Score of 0.053, showed a balance between consistency and peak performance. Model 3 revealed a solid performance, having a mean CV Brier Score of 0.092 and a notably lower lowest Brier Score of 0.045, indicating effective accuracy with potential for exceptional outcomes. Model 4, although exhibiting the highest mean CV Brier Score at 0.09469706, displayed promising potential with a lowest Brier Score of 0.0518. Lastly, Model 5 had a moderate mean CV Brier Score of 0.09075372 and the highest lowest Brier Score of 0.071, suggesting it was less consistent and had lower peak performance compared to the other models.
...
stepwise_model_list <- lapply(train_sets,FUN = function(train_set){
  Stepwise_models(train_set = train_set,myformula = formula_fixed,cv=TRUE)
})
...
The model evaluation metrics @table-metrics further substantiates their performance. The Lasso model, with a Hosmer-Lemeshow p-value of 0.389, and the Stepwise model, with a p-value of 0.330, both indicate well-fitting models. The chi-squared statistics, 8.466 for Lasso and 9.155 for Stepwise, are in close proximity to each other, suggesting that the differences in the predicted probabilities and the actual outcomes are not significant. Additionally, the Brier Scores, 0.077 for Lasso and 0.077 for Stepwise, confirm the accuracy of the models, with the Stepwise model displaying a marginally better score.
...
x_vars <- model.matrix(object = formula_fixed, data = completed_validation_long)
...
The variable importance analysis for the Lasso and Stepwise logistic regression models, as summarized by their coefficients and significance levels , reveals the factors most influential in predicting the outcome variable.
